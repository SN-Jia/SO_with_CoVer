...
from .generator import CoherenceBeam
...
class BertForOrdering(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None,
        ...
        for t in range(num):
            ...
            forw_pw = rela_vec_diret.clone().mean(2) 
            back_pw = rela_vec_diret.clone().mean(1)
...
def beam_search_pointer(args, model, graph_model, texts, input_ids, attention_mask=None, token_type_ids=None,
                pairs_list=None, passage_length=None, pairs_num=None, sep_positions=None, ground_truth=None, 
                mask_cls=None, pairwise_labels=None, cuda=None, head_mask=None):
    ...
    for t in range(target_t):
        ...
        next_beam = CoherenceBeam(valid_size)
        done_list, remain_list = next_beam.step(graph_model, args, texts, log_prob, prev_beam, f_done)
    ...
    sort_score, sort_ix = torch.sort(score, descending=True)
    output = []
